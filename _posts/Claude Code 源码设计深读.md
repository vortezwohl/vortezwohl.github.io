...
我读完 Claude Code 这一套源码后，最大的感受不是“它会调用很多工具”，而是：**它一直在防模型出错，防模型乱来，防环境反过来坑系统**。

如果用一句更直白的话概括：

**Claude Code 不是把模型直接放进终端里干活，而是先给模型套上一层很厚的“护栏”，模型只能在护栏里面行动。**

这层“护栏”，就是我理解里的 harness。

它不是某一个单独文件，也不是某一个类，而是一整套运行规则。它做的事情很像一个经验很重的工程主管：
...
所以，Claude Code 真正强的地方，不只是“会干活”，而是“**会在不完全相信模型的前提下，让模型尽量安全地干活**”。

Paper 级别的话可以说这是 agent runtime、policy enforcement、verification pipeline；但如果换成更容易懂的话，就是：

**它默认模型会犯错，所以系统从一开始就按“不信任模型”来设计。**

## 它到底在解决什么问题

先别急着看类名和模块名。先看它要面对的现实问题。
...
也就是说，在 Claude Code 里，模型不是“直接执行工具的人”，模型更像是“提申请的人”，真正能不能执行，要过系统检查。
...
**如果某个工具已经发起了调用，但后面流程崩了，那也要补一个失败结果，不能让记录悬在半空。**

这很重要，因为 Claude Code 很清楚：  
不是只有“成功执行”才重要，**失败时能不能把状态补完整**也同样重要。
...
**无论你权限模式怎么调，在交互模式下，都要先过“这个工作区值不值得信任”这一关。**

这不是普通权限确认框，而是更靠前的一层边界判断。
...
## 我理解的 Harness：不是帮模型更自由，而是让模型更受控

如果一定要解释 harness，我会这样说：

**harness 就是一层负责“接住模型输出、检查模型意图、限制模型行动、补偿模型失误”的系统外壳。**

它的核心不是提高模型自由度，而是控制模型边界。

换成更具体的说法，Claude Code 的 harness 主要在做三件事。
...
也就是说，模型输出进入系统后，先经过一层层处理，最后才会变成真正动作。
...
## Claude Code 是怎么在“对模型零信任”的基础上构建的
...
**即使按设计，模型本来就不该传这个字段，系统还是再手动剥掉一次。**
...
**就算 hook 说能过，也不能直接算数。还要继续看全局规则是不是反对。**

这意味着系统不是把权限交给某个单点，而是用多层规则相互制衡。

### 4. 不相信“自动模式”一定安全

Claude Code 支持 auto mode，也就是某些场景下系统自动判断是否批准，不总是打扰用户。
...
这就是为什么我说 Claude Code 把“目录是否可信”提升成了系统级问题。
...
**项目自己的配置不允许替用户自动同意危险设置，因为恶意项目可以借这个机会直接把系统带歪。**

这是非常好的工程意识。

因为如果项目配置也和用户本地配置一样有权，那任何人只要往仓库里塞一份配置文件，就等于替用户点了“允许”。

Claude Code 明显不接受这种风险。

## 沙箱不是“可选增强”，而是硬边界的一部分

Claude Code 的 sandbox 设计，给我的感觉不是“有个安全功能”，而是“它在主动补环境层的漏洞”。
...
因为一旦模型能写配置文件，很多限制都可能被反向篡改。

### 2. 不让模型随便写 `.claude/skills`

因为 skill 本身就很有能力。  
如果一个系统允许模型自动改 skill，本质上就是让它改自己的能力边界。
...
这已经不只是“我会做权限控制”，而是“我知道真实系统会在哪些地方漏风”。

## Bash 部分为什么特别值得研究

如果说 Claude Code 源码里最能体现“零信任”和“防御性工程”的地方，我认为是 Bash 模块。

因为 shell 是最危险、最容易被模型破坏的地方。

Claude Code 对 Bash 的态度不是：
...
所以 Claude Code 宁可花很多力气修补轨迹，也不愿意带着半坏状态继续往前跑。
...
**你不能只是说“这是结果”，你必须通过指定工具提交结果。**

也就是说，Claude Code 更相信“机器可检查的结果”，而不是“模型自称的结果”。

### 2. 多 agent 场景下，验证是独立的一层
...
这其实是在把“写代码”和“证明确实写对了”拆成两个阶段。

对 agent 系统来说，这一步非常重要。

因为模型最擅长的一件事，就是把“看起来像做完了”伪装成“真的做完了”。

## Claude Code 最值得学的地方

如果让我总结成几条最值得学的设计经验，我会写下面这些。
...
不是让模型更会说，而是让模型交出**可以检查的结果**。

## 我的理解

读完 Claude Code 源码后，我对它最本质的理解是：

**它不是在努力把模型训练成一个完全可靠的工程师，而是在搭一个足够结实的工程系统，在零信任的假设下，让一个完全不稳定的模型在受控边界内工作。**
...
我认为 Claude Code 走的是第二条路，而且这正是它源码最有价值的地方。

它真正成熟的地方，不在于“功能很多”，而在于它始终在追问这些更难的问题：
...
所以，如果一定要给 Claude Code 下一个最简洁的定义，我会写：

**Claude Code 是一个把不可信的大模型关进工程护栏里的 agent 运行时。**
